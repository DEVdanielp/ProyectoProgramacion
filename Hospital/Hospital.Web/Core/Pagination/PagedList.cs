using Hospital.Web.Data.Extensions;
...
namespace Hospital.Web.Core.Pagination
{
    public class PagedList<T>: List<T>
    {
        ...
        public PagedList(List<T> items, int count, int pageNumber, int recordPerPage)
        {
            TotalCount = count;
            RecordsPerPage = recordPerPage;
            CurrentPage = pageNumber;
            TotalPages = (int)Math.Ceiling(count / (double)recordPerPage);
            AddRange(items);
        }
